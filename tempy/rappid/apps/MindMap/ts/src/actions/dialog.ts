...
import { IdeaView } from '../shapes/idea';
import { getFocusableTextFieldElements } from '../utils';

const THEME = 'mind-map';

let _dialog: ui.Dialog = null;

export function openDialog(app: App, options: Partial<ui.Dialog.Options>): ui.Dialog {
    const { keyboard } = app;
    closeDialog(app);
...
export function showHelpDialog(app: App) {
...
export function showURLDialog(app: App, ideaView: IdeaView, annotationUrl: string) {
    ...
    const inspectorModel = new Model({
