import { anchors } from '@clientio/rappid';
import { IdeaView } from './idea';

const anchor: anchors.Anchor = (elementView: IdeaView, _node, _ref, _opt, end, linkView) => {
    ...
    switch (idea.get('layout')) {
        ...
        case 'L': {
            // Left Side Idea
            if (!isLine) return bbox.rightMiddle();
            return isSource ? bbox.bottomLeft() : bbox.bottomRight();
        }
        case 'R':
        default: {
            // Right Side Idea
            if (!isLine) return bbox.leftMiddle();
