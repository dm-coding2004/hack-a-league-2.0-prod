import { dia, V, ui, attributes as diaAttributes, util } from '@clientio/rappid';
...
interface IdeaLabelCache extends IdeaSectionCache {
    x: number;
    y: number;
}

interface IdeaCache {
    layout?: IdeaSectionCache;
    label?: IdeaSectionCache;
    image?: IdeaSectionCache;
}
...
export class Idea extends Element {
    ...
    static sandbox: SVGElement | null;
    ...
    defaults() {
        return {
            ...
            [SPACING_PROPERTY]: 5,
            [LABEL_PROPERTY]: '',
            [ANNOTATIONS_PROPERTY]: [] as ui.TextEditor.URLAnnotation[],
            [IMAGE_PROPERTY]: '',
        ...
        let $label;
        {
            ...
            if ('label' in cache) {
                ...
                const { width: textWidth, height: textHeight } = measureText(
                    Idea.sandbox,
                    attributes[LABEL_PROPERTY] || attributes[PLACEHOLDER_PROPERTY],
                    labelAttributes,
                    attributes[ANNOTATIONS_PROPERTY]
                );
                w = textWidth;
                h = textHeight;
                cache.label = {
    ...
    removeImage(opt?: dia.Cell.Options) {
        this.set(IMAGE_PROPERTY, null, opt);
    }

    addImage(imageUrl: string, opt?: dia.Cell.Options) {
        this.set(IMAGE_PROPERTY, imageUrl, opt);
    }

    // Annotation API

    replaceAnnotationURL(index: number, url: string, urlText: string, opt?: dia.Cell.Options) {
    ...
    get annotations(): ui.TextEditor.URLAnnotation[] {
...
export class IdeaView extends ElementView {
    ...
    updateBody($body: IdeaSectionCache) {
        const { model, vBody } = this;
    ...
    updateImage($image: IdeaImageCache) {
    ...
    updateLabel($label: IdeaLabelCache) {
