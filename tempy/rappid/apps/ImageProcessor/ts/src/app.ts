import { anchors, connectors, dia, highlighters, shapes, ui } from '@clientio/rappid';
import { StencilService } from './stencil/stencil-service';
import { Processor } from './processor/processor';
import { Connection, ConnectionSource, ConnectionTarget } from './connection/connection';
...
export class App {
    ...
    constructor() {
        ...
        this.paper = new dia.Paper({
            ...
            validateConnection: (
                ...
                if (sourceConnectedLinks.some(l => targetConnectedLinks.includes(l))) return false;
        ...
        this.cmd = new dia.CommandManager({
            ...
            cmdBeforeAdd: (cmdName, _cell, _graph, options = {}) => {
                if (options.inspector || options.input) return true;
    ...
    setInteractivity() {
        ...
        let cellViewStack: dia.CellView[] = [];
        ...
        const onElementHover = (elementView: dia.ElementView) => {
        ...
        const onElementHoverEnd = (elementView: dia.ElementView) => {
            if (elementView instanceof NodeView) {
        ...
        const onLinkHover = (linkView: dia.LinkView) => {
            linkView.el.classList.add('connection-hover');
        }

        const onLinkHoverEnd = (linkView: dia.LinkView) => {
        ...
        this.keyboard.on({
            ...
            'delete backspace': (_evt: dia.Event) => {
        ...
        paper.on('cell:mouseenter', (cellView, _evt) => {
            ...
            if (current) {
                if (current.model.isElement()) {
                    onElementHoverEnd(current as dia.ElementView);
                }

                if (current.model.isLink()) {
                    onLinkHoverEnd(current as dia.LinkView);
                }
            }
            ...
            if (cellView.model.isElement()) {
                onElementHover(cellView as dia.ElementView);
            }

            if (cellView.model.isLink()) {
                onLinkHover(cellView as dia.LinkView);
            }
        });

        paper.on('cell:mouseleave', (cellView, _evt) => {
            if (cellView.model.isElement()) {
                onElementHoverEnd(cellView as dia.ElementView);
            }

            if (cellView.model.isLink()) {
                onLinkHoverEnd(cellView as dia.LinkView);
            }
            ...
            if (current) {
                if (current.model.isElement()) {
                    onElementHover(current as dia.ElementView);
                }

                if (current.model.isLink()) {
                    onLinkHover(current as dia.LinkView);
                }
            }
        });
        ...
        paper.on('link:connect', (linkView) => {
            ...
            let outputNode: Node;
            if (targetGroup === 'in') {
                inputNode = targetModel as Node;
                outputNode = sourceModel as Node;

                inputIndex = Number(targetMagnet.getAttribute('port-index'));
                outputIndex = Number(sourceMagnet.getAttribute('port-index'));
            } else {
                inputNode = sourceModel as Node;
                outputNode = targetModel as Node;
    ...
    setContextToolbar() {
        ...
        const render = (point: joint.dia.Point, cells: joint.dia.Cell[] = []) => {
