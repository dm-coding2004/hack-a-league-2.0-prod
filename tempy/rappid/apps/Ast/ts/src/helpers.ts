...
export function highlightRange(range: [number, number]) {
    editorView.dispatch({
        effects: highlightMark.of([highlightDecoration.range(range[0], range[1])])
    })
}

function tokenHover(e: Event) {
    const li = e.target as HTMLLIElement;
    const range: [number, number] = JSON.parse(li.getAttribute('data-range'));
    unhighlightRange();
...
// codemirror
const highlightMark = StateEffect.define<Array<Range<Decoration>>>();
