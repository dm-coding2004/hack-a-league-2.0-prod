import { dia, ui, layout, V, elementTools, util } from '@clientio/rappid';
...
export class Yamazumi3D {
    ...
    createOperators() {
        ...
        this.operators.forEach((operator, operatorIndex) => {
            ...
            tasks.forEach((task, taskIndex) => {
                const taskElement = new TaskElement(Object.assign(Object.assign({}, task), { durationHeight: this.durationHeight, stackIndex: operatorIndex, stackElementIndex: taskIndex, depth: this.taskDepth }));
    ...
    drawTakt() {
        ...
        const x = this.topLeft.x - this.legendOffset;
        const layer = this.paper.getLayerNode(dia.Paper.Layers.FRONT);
        return V('path', {
