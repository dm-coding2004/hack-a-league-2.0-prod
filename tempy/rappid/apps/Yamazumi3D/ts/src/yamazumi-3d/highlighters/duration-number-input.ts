import { dia, ui, util } from '@clientio/rappid';
...
export class DurationNumberInput extends dia.HighlighterView<DurationNumberInputOptions> {
    ...
    events() {
        return {
            'change input': (evt: dia.Event) => { this.onChange(evt); },
            'click input': (evt: dia.Event) => { this.onClick(evt); },
            'focus input': (evt: dia.Event) => { this.onFocus(evt); },
            'blur input': (evt: dia.Event) => { this.onBlur(evt); }
        }
    }
    ...
    onFocus(_evt: dia.Event) {
        this.cellView.model.set('focused', true);
    }

    onBlur(_evt: dia.Event) {
