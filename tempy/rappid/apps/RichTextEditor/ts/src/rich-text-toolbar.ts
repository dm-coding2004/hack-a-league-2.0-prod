...
import { ui, mvc, attributes, util } from '@clientio/rappid';
...
interface RichTextToolbarOptions extends mvc.ViewOptions<undefined, HTMLDivElement> {
    ...
    setAttributes?: (attributes: attributes.SVGAttributes) => void;
}

export class RichTextToolbar extends mvc.View<undefined, HTMLDivElement> {

    options: RichTextToolbarOptions;

    textColorInput: ui.ColorPalette;
    textStyleInput: ui.SelectButtonGroup;
    fontSizeInput: ui.SelectBox;
    fontFamilyInput: ui.SelectBox;
    clearFormattingInput: HTMLButtonElement;

    fontWeightUndefined: boolean;
    fontStyleUndefined: boolean;
    textDecorationUndefined: boolean;

    constructor(options: RichTextToolbarOptions) {
        super(options);
    }
    ...
    resetAnnotation() {
        ...
        const { options } = this;
        const { defaultAttributes = {}, setAttributes = () => { /* noop */ } } = options;
    ...
    updateInputs(attrs: attributes.SVGAttributes) {
    ...
    updateInputsDebounced = util.debounce((attrs: attributes.SVGAttributes) => {
        this.updateInputs(attrs);
    }, 50)
}
