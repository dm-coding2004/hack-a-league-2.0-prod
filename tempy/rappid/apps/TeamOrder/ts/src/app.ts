import { dia, ui, layout } from '@clientio/rappid';
import { members } from './data';
import { Member, Link } from './shapes';
import { Member as MemberType } from './models';
...
export const init = () => {
    ...
    const paper = new dia.Paper({
        ...
        defaultAnchor: (elementView) => {
            const element: dia.Cell = elementView.model;
    ...
    const stackLayoutView: ui.StackLayoutView = new ui.StackLayoutView({
        paper: paper,
    ...
    const toolbar = new ui.Toolbar({
        el: document.querySelector('#toolbar') as HTMLElement,
        tools: [
...
const addTeam = (teamMembers: MemberType[], graph: dia.Graph, paper: dia.Paper): void => {
...
const shuffleTeam = (stackLayoutView: ui.StackLayoutView): void => {
