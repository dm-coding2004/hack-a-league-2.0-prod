...
export const init = () => {
    ...
    const paper = new dia.Paper({
        ...
        validateConnection: function(sv, sm, tv, tm, end) {
            const svModel = sv.model as Records;
            const tvModel = tv.model as Records;
            if (sv === tv) return false;
            if (svModel.isLink() || tvModel.isLink()) return false;
            if (end === 'target') {
                const targetItemId = tv.findAttribute('item-id', tm);
                if (!tvModel.isItemInView(targetItemId)) return false;
                return (tvModel.getItemSide(targetItemId) !== 'right');
            }
            const sourceItemId = sv.findAttribute('item-id', sm);
            if (!svModel.isItemInView(sourceItemId)) return false;
    ...
    const commandManager = new dia.CommandManager({
        ...
        cmdBeforeAdd: function(eventName) {
            if (eventName === 'change:scrollTop') return false;
            return true;
        }
    });
    ...
    paper.on('link:mousewheel', (_, evt: dia.Event, ox: number, oy: number, delta: number) => {
        evt.preventDefault();
        zoom(ox, oy, delta);
    });
    ...
    paper.on('element:magnet:pointerdblclick', (elementView: dia.ElementView, evt: dia.Event, magnet: Element) => {
        evt.stopPropagation();
        const model = elementView.model as Records;
    ...
    paper.on('element:magnet:contextmenu', (elementView: dia.ElementView, evt: dia.Event, magnet: Element) => {
        const model = elementView.model as Records;
    ...
    paper.on('element:pointerup', (view: dia.ElementView, evt: dia.Event, x: number, y: number) => {
        const data = evt.data;
        if (data.ghost) {
            data.ghost.remove();
    ...
    paper.on('element:mousewheel', (recordView: dia.ElementView, evt: dia.Event, x: number, y: number, delta: number) => {
        evt.preventDefault();
        const record = recordView.model as Records;
        if (!record.isEveryItemInView()) {
            record.setScrollTop(record.getScrollTop() + delta * 10);
        }
    });

    paper.on('element:decorator:pointerdown', (recordView: dia.ElementView, evt: dia.Event, itemId: string) => {
        const record = recordView.model as Records;
    ...
    function showElementTools(elementView: dia.ElementView) {
